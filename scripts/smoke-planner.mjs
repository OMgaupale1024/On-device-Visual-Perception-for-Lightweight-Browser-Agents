...
const expectedMode = process.argv.includes('--ai') ? 'ai' : 'deterministic';
assert.equal(click.plannerMode, expectedMode);
...
assert.equal(stop.plannerMode, expectedMode);
assert.equal(stop.plan.target, null);
console.log(`PASS ${expectedMode} localhost HTTP: health, CLICK, STOP, observation binding; approved payload ${click.bytes} bytes.`);
