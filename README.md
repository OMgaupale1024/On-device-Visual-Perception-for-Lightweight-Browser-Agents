...
```text
extension/    Chrome MV3 extension (local OCR, privacy, actions, verification)
server/       FastAPI planner server (deterministic + NVIDIA NIM)
dashboard/    EdgeSight Analytics — standalone React/Vite dashboard
benchmarks/   Controlled prototype fixtures and reference results
scripts/      Root tooling (build, check, secret scan, benchmark)
demo-page/    Synthetic travel-request form used to exercise perception
demo/         Synthetic search page
docs/         Architecture, decisions, testing and metrics
```

This root package owns the extension and its tooling; the commands above run from
here. The dashboard is a **separate package with its own dependencies and lockfile**
— see [dashboard/README.md](dashboard/README.md) and build it from `dashboard/`:
...
The dashboard does not consume `scripts/benchmark.mjs` output; its import schema and
the benchmark schema are not currently compatible.
